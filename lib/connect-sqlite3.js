...
module.exports = function(connect) {
    ...
    */
	...
	store.db.all('SELECT * FROM ? WHERE ? > expired', [store.table, now], function(err, res) {
	    if(!err)
		store.client.emit('cleaup', res);
	    store.db.run('DELETE FROM ? WHERE ? > expired', [store.table, now]);
	});
    }
